...
Roughly speaking, they are mathematical objects glued from zero loci of polynomials which globally satisfy a certain Hausdorffness property.
...
From now on we follow \cite{har77} and assume all varieties to be irreducible unless otherwise specified.
...
\begin{exa}
    If $X$ is a curve, then any cycle $\xi\in Z(X)$ has the form
    \[ n_{1}P_{1}+\ldots+n_{r}P_{r}+m X, \]
    where $r\in \N$, $n_{1},\ldots,n_{r},m\in \Z$ and $P_{1},\ldots,P_{r}\in X$ are points.
\end{exa}
...
\begin{rem}\mbox{}
	...
	\item In particular, linear and rational equivalence agree for divisors and
	    \[ A^{1}(X)=\Cl(X). \]
	    This explains ---I think--- the intuition for varying an effective divisor within its linear equivalence class as ``moving it around our variety''.
    \end{enumerate}
\end{rem}
